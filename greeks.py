...
def black_scholes_greeks(spot: float, strike: float, time_years: float, iv_percent: float,
    ...
    """
    Returns {"delta": ..., "gamma": ...}. delta is signed (negative for puts).
    Falls back to a sane flat estimate if inputs are degenerate (IV=0, T<=0 etc)
    rather than raising -- callers shouldn't have to special-case bad data.
    """
    ...
    if spot <= 0 or strike <= 0 or time_years <= 0 or iv_percent <= 0:
        ...
        return {"delta": flat, "gamma": 0.0, "valid": False}
    ...
    return {"delta": delta, "gamma": gamma, "valid": True}
