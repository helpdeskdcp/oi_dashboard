...
- The individual confidence factors were then isolated and tested -- see `CONFIDENCE_FACTOR_ISOLATION_REPORT.md`. First surfaced a structural gap: `backtest.simulate_trades()` never passes `market_structure`/`nse_atm_row` to `generate_signal()`, so 5 of its 9 confidence bonuses (structural proximity, regime alignment, cross-verify-wall, dual-source agreement, order-flow) have never fired in ANY backtest in this series, including the prior three reports -- their real predictive value is simply untested, not confirmed useless. Of the 4 factors that could be tested, none showed a clean positive edge, and PCR extremity -- the single most influential one, firing on 78% of all trades -- was associated with measurably *worse* outcomes (profit factor 0.53 fired vs. 0.67 not fired). No weight changed -- flagged as a candidate for a properly train/test-validated fix, and extending the backtest to actually exercise the untested 5 factors is flagged as the clearest next step.
